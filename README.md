...
- 提前给在智谱账户后台充值好余额
- 提前 20 分钟以上打开页面
- 遇到「抢购人数过多／系统繁忙」是服务器在限流，脚本会自动刷新重拉并逐步退避重试（放慢请求频率，降低触发 WAF 限流的概率）。
...
时间规则：脚本只看 `CONFIG` 里的时分秒。当天目标时刻（默认10:00:00，每天放库存的时刻）后 60 分钟内打开，抢当天（已过点就立即开抢，正好赶回流）。11:00:00 后打开脚本将自动改抢明天同一目标时刻。脚本会提前一会儿，在9:58开始自动刷新。
...
- 系统繁忙（接口 555／无套餐卡）时自动刷新重拉，并指数退避，避免被单独限流
- 自动捕获腾讯验证码图片并缓存
- 调用本地识别接口，自动点选并提交
...
- 验证码未通过且服务端仍是同一张图时自动「换一张」，不在旧图上反复撞同一个错误答案
